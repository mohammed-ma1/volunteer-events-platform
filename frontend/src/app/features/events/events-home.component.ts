...
@Component({
  ...
  template: `
    ...
    >
      ...
      <!-- Strategic-partner cards — float on the hero's left/right edges
           (desktop only; the centered content keeps full width on mobile).
           Outer wrapper handles the fade-up entrance, inner card the gentle
           perpetual float, mirroring the rest of the hero's motion. -->
      <div
        class="ve-hero-partner motion-safe:animate-ve-fade-up absolute left-6 top-1/2 z-10 hidden -translate-y-1/2 motion-safe:[animation-delay:320ms] xl:block 2xl:left-12"
      >
        <div
          class="motion-safe:animate-ve-float flex w-40 flex-col items-center gap-3 rounded-2xl border border-slate-200/70 bg-white/95 px-5 py-5 text-center shadow-[0_18px_44px_-22px_rgba(30,41,59,0.5)] backdrop-blur-sm xl:w-44"
        >
          <img
            src="/images/partners/national-fund.png"
            [alt]="i18n.t('hero.partnersLabel')"
            class="h-20 w-auto object-contain"
            loading="lazy"
          />
          <span class="inline-flex items-center gap-1.5 text-[0.72rem] font-semibold text-gold-700">
            <svg class="h-3 w-3 shrink-0 text-gold-500" fill="currentColor" viewBox="0 0 24 24" aria-hidden="true">
              <path
                d="M11.049 2.927c.3-.921 1.603-.921 1.902 0l1.519 4.674a1 1 0 00.95.69h4.915c.969 0 1.371 1.24.588 1.81l-3.976 2.888a1 1 0 00-.363 1.118l1.518 4.674c.3.922-.755 1.688-1.538 1.118l-3.976-2.888a1 1 0 00-1.176 0l-3.976 2.888c-.783.57-1.838-.197-1.538-1.118l1.518-4.674a1 1 0 00-.363-1.118L2.05 10.101c-.783-.57-.38-1.81.588-1.81h4.914a1 1 0 00.951-.69l1.519-4.674z"
              />
            </svg>
            {{ i18n.t('hero.partnersLabel') }}
          </span>
        </div>
      </div>

      <div
        class="ve-hero-partner motion-safe:animate-ve-fade-up absolute right-6 top-1/2 z-10 hidden -translate-y-1/2 motion-safe:[animation-delay:420ms] xl:block 2xl:right-12"
      >
        <div
          class="motion-safe:animate-ve-float flex w-40 flex-col items-center gap-3 rounded-2xl border border-slate-200/70 bg-white/95 px-5 py-5 text-center shadow-[0_18px_44px_-22px_rgba(30,41,59,0.5)] backdrop-blur-sm motion-safe:[animation-delay:1.2s] xl:w-44"
        >
          <img
            src="/images/partners/ooredoo.png"
            [alt]="i18n.t('hero.partnersLabel')"
            class="h-20 w-auto object-contain"
            loading="lazy"
          />
