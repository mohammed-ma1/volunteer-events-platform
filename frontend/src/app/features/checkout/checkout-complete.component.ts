    ...
    this.checkoutApi
      ...
      .subscribe({
        next: (o) => {
          this.order.set(o);
          if (o.status === 'paid') {
            // Successful payment: drop the cart so the next visit starts
            // fresh. Fall back to local-only clear if the API call fails so
            // the UI never gets stuck showing paid items.
            if (this.cart.token()) {
